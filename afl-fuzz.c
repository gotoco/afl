...
//static int fd_afl_area;               /* FD for external fuzzing devices  */
...
#if defined(__NetBSD__)
EXP_ST void setup_shm_kernel(void) {
  uint64_t kcov_bsize = MAP_SIZE;

  /* NetBSD specific TODO: merge with Linux */
  fd_kernel_device = open("/dev/kcov", O_RDWR);
  if (fd_kernel_device < 0)
    PFATAL("open(/dev/kcov) failed");

  if (ioctl(fd_kernel_device, KCOV_IOC_SETBUFSIZE, &kcov_bsize) == -1)
    PFATAL("ioctl: KCOV_IOC_SETBUFSIZE failed");
...
static void afl_start_tracing(void) {
    ...
    if (ioctl(fd_kernel_device, KCOV_IOC_ENABLE, &mode) == -1)
            PFATAL("ioctl: KCOV_IOC_ENABLE");
}
#endif
...
static void afl_stop_tracing(void) {
  ...
  if (ioctl(fd_kernel_device, KCOV_IOC_DISABLE) == -1)
    PFATAL("ioctl: KCOV_IOC_DISABLE");
}
#endif
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    setsid();
/* Comment for debugging and verbosity purposes */
//    dup2(dev_null_fd, 1);
//    dup2(dev_null_fd, 2);
...
/* TODO: Part of that needs to be moved to OS layer! */
#if defined(__linux__)
...
#endif
#if defined(__NetBSD__)
ACTF("Fuzz the planet, or at least kernel!\n");
        /* Already set */
        if (kernel_mode)
          break;

        if (file_exists("/dev/kcov")) {
          kernel_mode = 1;
          OKF("Using /dev/kcov as coverage device.");
        } else {
          FATAL("/dev/kcov not found in the running kernel\n"
                "Reconfigure your kernel with KCOV to do kernel fuzzing!");
          break;
        }
#endif
      case 'S':
